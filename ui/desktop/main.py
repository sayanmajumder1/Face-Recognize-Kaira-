...
def search_documents():
    year = year_combobox.get() # type: ignore
    month = month_combobox.get() # type: ignore
    day = day_combobox.get() # type: ignore
    file_name = file_name_entry.get() # type: ignore
...
def show_splash():
    ...
    splash_label = tk.Label(splash, text="WelCome In Mirror ...", font=("Courier", 18, "bold"), bg='#0a0a0a', fg='#00ff00')
    splash_label.pack(expand=True)

    # Hide the splash screen after 3 seconds and show the main window
    root.after(4000, lambda: (splash.destroy(), show_main_window()))
...
# Show the splash screen and then the main window
root = tk.Tk()  # Temporary root window needed for splash screen
root.withdraw()  # Hide it initially

show_splash()  # Call the function to display the splash screen
